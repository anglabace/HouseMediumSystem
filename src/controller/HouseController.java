...
//		r.setFloor_("2");
	 ...
	 */
		 ...
		 * 根据客户需求快速匹配房源
		 */
		@RequestMapping("findHouseRsesourceByClientReq")
		public String findHouseRsesourceByClientReq(int staff_id,int application_id,float minPrice,float maxPrice,String siteRequirement,@Param("flag")int flag,ModelMap map){
			//找到所有房屋用途下拉选项框
			List<HouseApplication> houseAppSelection = houseApplicationService.findAllHouseAppSelection();
			//找到所有房屋装饰下拉选项框
			List<HouseAdornment> houseAdorSelection = houseAdornmentService.findAllAdorSelection();
			//找到所有房屋朝向下拉选项框
			List<HouseDirection> houseDireSelection = houseDirectionService.findAllHouseDirSelection();
			//因为单位为万 但是租房需求信息填写的的为小数点 所以乘以10000
			if(flag == 2){
				minPrice = minPrice*10000;
				maxPrice = maxPrice*10000;
			}
			List<HouseResource> houseList = houseService.findHouseRsesourceByClientReq(staff_id, application_id, minPrice, maxPrice, siteRequirement, flag);
			map.put("houseList",houseList);
			map.put("houseAppSelection", houseAppSelection);
			map.put("houseAdorSelection", houseAdorSelection);
			map.put("houseDireSelection", houseDireSelection);
