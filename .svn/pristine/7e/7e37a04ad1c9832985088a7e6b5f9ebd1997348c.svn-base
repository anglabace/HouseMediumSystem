						...
						<ul>
							...
							<li>
								<label>
									...
									<select id="servicearea" class="inputname" name="area_Id">
										<c:forEach items="${areaList }" var="a">
											<c:if test="${a.area_Id==area.area_Id }">
												<option value="${a.area_Id }" selected>${a.area_Name }</option>
											</c:if>	
											<c:if test="${a.area_Name!=area.area_Name }">
